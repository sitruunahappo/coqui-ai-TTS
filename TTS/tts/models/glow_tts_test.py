...
class GlowTTS(BaseTTS):
    ...
    def inference_with_MAS(self, x, x_lengths, y=None, y_lengths=None, pitch=None, aux_input: dict[str, Any] | None = None):
        """
    ...
    @torch.inference_mode()
    def decoder_inference(self, y, y_lengths=None, pitch=None, aux_input={"d_vectors": None, "speaker_ids": None}):  # pylint: disable=dangerous-default-value
        """
        Shapes:
            - y: :math:`[B, T, C]`
            - y_lengths: :math:`B`
    ...
    @torch.inference_mode()
    def inference(self, x, pitch=None, aux_input: dict[str, Any] | None = None):
        if aux_input is None:
        ...
        '''
        if pitch is None:
            id2pitch = torch.load('/root/cantonese_daily/map.pt', map_location=torch.device("cpu"))
            pitch = torch.zeros_like(z) #(z.shape[0], z.shape[1], z.shape[2] / 2)
            B, T = x.shape
            ...
            for i in range(B):
                for j in range(T):
                    ...
                    try:
                        pitch[i][j] = id2pitch[this_tok][last_tok][next_tok]
                    except:
                        pitch[i][j] = id2pitch[0][last_tok][next_tok]
    ...
    def get_data_loader(
        ...
        dataset = TTSDataset(
            ...
            compute_f0=config.get("compute_f0", True),
